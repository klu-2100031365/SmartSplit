...
const ActivitiesMoviesPage = () => {
    ...
    const handleCreateEvent = async (name: string, icon: string, image?: string) => {
        if (!user) return;
        try {
            await api.createTrip(user.id, name, icon, image, 'movies');
            refresh();
                        ...
                        <div className="flex items-center gap-4 min-w-0">
                            ...
                            <div className="min-w-0">
                                <h1 className="text-3xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                                    <Film className="text-brand-purple" /> Movies
                                </h1>
                        ...
                        {isLoading ? (
                            ...
                            events.map(event => (
                                <ActivityEventCard
                                    ...
                                    symbol={symbol}
                                    onClick={() => router.push(`/activities/movies/${slugify(event.name)}`)}
...
export default ActivitiesMoviesPage;
